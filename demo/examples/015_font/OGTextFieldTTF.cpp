...
bool TextFieldDelegate::onVisit(TextFieldTTF* /*sender*/, Renderer* /*renderer*/, const Mat3& /*transform*/, uint32_t /*flags*/)
{
    return false;
}
...
#if (OG_TARGET_PLATFORM == OG_PLATFORM_MAC || OG_TARGET_PLATFORM == OG_PLATFORM_WIN32 || OG_TARGET_PLATFORM == OG_PLATFORM_LINUX)
...
bool TextFieldTTF::attachWithIME()
{
    ...
    {
        // open keyboard
        auto pSDLView = Director::getInstance()->getOpenSDLView();
        if (pSDLView)
        {
			pSDLView->setIMEKeyboardState(true);
        }
    }
    return ret;
}
...
bool TextFieldTTF::detachWithIME()
{
    ...
    {
        // close keyboard
        auto sdlView = Director::getInstance()->getOpenSDLView();
        if (sdlView)
        {
			sdlView->setIMEKeyboardState(false);
        }
    }
    return ret;
}
...
void TextFieldTTF::visit(Renderer *renderer, const Mat3 &parentTransform, uint32_t parentFlags)
{
