...
// #include "OGFontFNT.h"
// #include "ccShaders.h"
// #include "backend/ProgramState.h"
...
public:
    ...
    static LabelLetter* createWithTexture(Texture2D *texture, const Rect& rect=rect::ZERO, bool rotated = false)
    {
	...
	{
		OGASSERT(texture != nullptr, "Invalid texture for LabelLetter");
		bool result = false;
		if (Node::init())
		{ 
	...
	void  setTexture(Texture2D* texture, const Rect & clip)
	{
	...
	virtual void updateTransform() 
	{
		//if (isDirty())
		{
			_transformTo = getNodeToParentTransform();
			Size &size = _clip.size;
			...
			// issue #17022: don't flip, again, the letter since they are flipped in LabelLetter's code
			//if (_flippedX) std::swap(x1, x2);
			//if (_flippedY) std::swap(y1, y2);

			float x = _transformTo.tx;
			float y = _transformTo.ty;

			float cr = _transformTo.a;
			float sr = _transformTo.b;
			float cr2 = _transformTo.d;
			float sr2 = -_transformTo.c;
			...
			_quad.tl.vertices.set(ax,ay);
			_quad.tr.vertices.set(bx, by);
			_quad.bl.vertices.set(dx, dy);
			_quad.br.vertices.set(cx, cy);
    ...
    virtual void updateColor()  
    {
      ...
      //  auto displayedOpacity = _displayedOpacity;
		auto displayedOpacity = _displayedColor.a;
        if(!_letterVisible)
        {
	...
	void updateDisplayedColor(Color4B &r)
	{
		_displayedColor = r;
	}
    ...
    void draw(Renderer* /*renderer*/, const Mat3 & /*transform*/, uint32_t /*flags*/) override
    {
    }
...
private:
	Rect _clip;
	BlendFunc _bfunc;
	Vec2 _position,_offsetPosition;
    bool _letterVisible;
	int _vertCount = 4, _indexCount = 6;
...
// Label::BatchCommand::~BatchCommand()
// {
// //     OG_SAFE_RELEASE_NULL(textCommand.getPipelineDescriptor().programState);
// //     OG_SAFE_RELEASE_NULL(shadowCommand.getPipelineDescriptor().programState);
// //     OG_SAFE_RELEASE_NULL(outLineCommand.getPipelineDescriptor().programState);
// }
...
Label::~Label()
{
    ...
    if (_fontAtlas)
    {
        ...
        OG_SAFE_RELEASE_NULL(_reusedLetter);
        _batchNodes.clear();
        FontAtlasCache::releaseFontAtlas(_fontAtlas);
    }
  //  _batchCommands.clear();
    ...
    OG_SAFE_RELEASE_NULL(_textSprite);
    OG_SAFE_RELEASE_NULL(_shadowNode);
}
...
void Label::reset()
{
   ...
   // _batchCommands.clear();
    ...
    _systemFontSize = OG_DEFAULT_FONT_LABEL_SIZE;
...
//     {
//         OG_SAFE_RELEASE_NULL(_programState);
//         _programState = programState;
//         OG_SAFE_RETAIN(programState);
//     }
...
//         {
//             case orange::LabelEffect::NORMAL:
...
//                 }
//                 break;
//             case orange::LabelEffect::OUTLINE:
//                 {
//                     programType = backend::ProgramType::LABLE_OUTLINE;
//                 }
//                 break;
//             case orange::LabelEffect::GLOW:
//                 if (_useDistanceField)
//                 {
...
// {
//     OGASSERT(_programState, "programState should be set!");
// 
//     auto& pipelineDescriptor = batch.textCommand.getPipelineDescriptor();
//     OG_SAFE_RELEASE_NULL(pipelineDescriptor.programState);
...
//     OG_SAFE_RELEASE_NULL(pipelineShadow.programState);
...
//     OG_SAFE_RELEASE_NULL(pipelineOutline.programState);
...
void Label::setFontAtlas(FontAtlas* atlas,bool distanceFieldEnabled /* = false */, bool useA8Shader /* = false */)
{
    ...
    {
        _reusedLetter = LabelLetter::create();
...
bool Label::setBMFontFilePath(const std::string& bmfontFilePath, const Vec2& imageOffset, float fontSize)
{
    ...
    if (std::abs(fontSize) < FLT_EPSILON) {
        ...
        if (bmFont) {
            ...
            _bmFontSize = originalFontSize /*/ OG_CONTENT_SCALE_FACTOR()*/;
        }
    }
...
void Label::disableEffect(LabelEffect effect)
{
    switch (effect)
    {
        ...
        case orange::LabelEffect::OUTLINE:
            ...
            break;
        case orange::LabelEffect::SHADOW:
            if (_shadowEnabled)
            {
                _shadowEnabled = false;
                OG_SAFE_RELEASE_NULL(_shadowNode);
        ...
        case orange::LabelEffect::GLOW:
            ...
            break;
        case orange::LabelEffect::ITALICS:
            setRotationSkewX(0);
            break;
        case orange::LabelEffect::BOLD:
            if (_boldEnabled) {
                _boldEnabled = false;
            ...
            break;
        case orange::LabelEffect::UNDERLINE:
            if (_underlineNode) {
                removeChild(_underlineNode);
                _underlineNode = nullptr;
            }
            break;
        case orange::LabelEffect::STRIKETHROUGH:
            _strikethroughEnabled = false;
...
void Label::createSpriteForSystemFont(const FontDefinition& fontDef)
{
    ...
    _textSprite = LabelLetter::createWithTexture(texture);
...
void Label::createShadowSpriteForSystemFont(const FontDefinition& fontDef)
{
    ...
    {
        _shadowNode = LabelLetter::createWithTexture(_textSprite->getTexture());
    }
    else
    {
        ...
        _shadowNode = LabelLetter::createWithTexture(texture);
        texture->release();
    }
...
// }
// 
// void Label::updateEffectUniforms(BatchCommand &batch, TextureAtlas* textureAtlas, Renderer *renderer, const Mat3 &transform)
// {
...
//             {
//                 int effectType = 0;
// 				Color4F effectColor(_effectColorF.r, _effectColorF.g, _effectColorF.b, _effectColorF.a);
//                 
//                 //draw shadow
//                 if(_shadowEnabled)
//                 {
//                     effectType = 2;
//                     Color4F shadowColor = Color4F(_shadowColor4F.r, _shadowColor4F.g, _shadowColor4F.b, _shadowColor4F.a);
//                     auto *programStateShadow = batch.shadowCommand.getPipelineDescriptor().programState;
//                     programStateShadow->setUniform(_effectColorLocation, &shadowColor, sizeof(Color4F));
...
//                     programStateOutline->setUniform(_effectColorLocation, &effectColor, sizeof(Color4F));
...
// 					Color4F shadowColor = Color4F(_shadowColor4F.r, _shadowColor4F.g, _shadowColor4F.b, _shadowColor4F.a);
//                     auto *programStateShadow = batch.shadowCommand.getPipelineDescriptor().programState;
//                     programStateShadow->setUniform(_textColorLocation, &shadowColor, sizeof(Color4F));
//                     programStateShadow->setUniform(_effectColorLocation, &shadowColor, sizeof(Color4F));
//                     batch.shadowCommand.init(_globalZOrder);
//                     renderer->addCommand(&batch.shadowCommand);
//                 }
...
#endif
    {
        orange::Mat3 matrixProjection = Director::getInstance()->getMatrix(MATRIX_STACK_TYPE::MATRIX_STACK_PROJECTION);
        ...
        {
            orange::Mat3 matrixMVP = matrixProjection * transform;
...
void Label::visit(Renderer *renderer, const Mat3 &parentTransform, uint32_t parentFlags)
{
    ...
    // IMPORTANT:
    // To ease the migration to v3.0, we still support the Mat3 stack,
...
{
//    OGASSERT(_currentLabelType != LabelType::STRING_TEXTURE, "Not supported system font!");
...
void Label::setAdditionalKerning(float space)
{
	...
	if (_currentLabelType != LabelType::STRING_TEXTURE)
	{
		...
		;// OGLOG("Label::setAdditionalKerning not supported on LabelType::STRING_TEXTURE");
}
...
{
 //   OGASSERT(_currentLabelType == LabelType::TTF || _currentLabelType == LabelType::STRING_TEXTURE, "Only supported system font and ttf!");
...
void Label::updateColor()
{
    ...
    orange::TextureAtlas* textureAtlas;
...
#if (OG_TARGET_PLATFORM != OG_PLATFORM_ANDROID) && (OG_TARGET_PLATFORM != OG_PLATFORM_IOS)
    if (systemFontDef._stroke._strokeEnabled)
    {
        OGLOGERROR("Stroke Currently only supported on iOS and Android!");
    }
    systemFontDef._stroke._strokeEnabled = false;
#endif
...
void Label::computeAlignmentOffset()
{
	...
	switch (_vAlignment)
	{
	case orange::TextVAlignment::TOP:
		_letterOffsetY = _contentSize.height;
		break;
	case orange::TextVAlignment::CENTER:
		_letterOffsetY = (_contentSize.height + _textDesiredHeight) / 2.f;
		break;
	case orange::TextVAlignment::BOTTOM:
...
void Label::updateBMFontScale()
{
	...
	if (_currentLabelType == LabelType::BMFONT) {
		...
		float originalFontSize = bmFont->getOriginalFontSize();
		_bmfontScale = _bmFontSize/* * OG_CONTENT_SCALE_FACTOR() */ / originalFontSize;
	}
	else {
		_bmfontScale = 1.0f;
	}
...
bool Label::multilineTextWrap(const std::function<int(const std::u32string&, int, int)>& nextTokenLen)
{
	...
	for (int index = 0; index < textLen; )
	{
		...
		for (int tmp = 0; tmp < tokenLen; ++tmp)
		{
			...
			{
				recordPlaceholderInfo(letterIndex, character);
				//OGLOG("LabelTextFormatter error: can't find letter definition in font file for letter: 0x%x", character);
				continue;
			}
...
void Label::recordLetterInfo(const orange::Vec2& point, char32_t utf32Char, int letterIndex, int lineIndex)
{
