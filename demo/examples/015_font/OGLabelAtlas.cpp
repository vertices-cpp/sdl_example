...
bool LabelAtlas::initWithString(const std::string& theString, const std::string& fntFile)
{
  ...
  //  OGASSERT(dict["version"].asInt() == 1, "Unsupported version. Upgrade orange version");
    ...
    unsigned int width = dict["itemWidth"].asInt() /*/ OG_CONTENT_SCALE_FACTOR()*/;
    unsigned int height = dict["itemHeight"].asInt()/* / OG_CONTENT_SCALE_FACTOR()*/;
...
void LabelAtlas::updateAtlasValues()
{
    ...
    float itemWidthInPixels = _itemWidth /** OG_CONTENT_SCALE_FACTOR()*/;
    float itemHeightInPixels = _itemHeight/* * OG_CONTENT_SCALE_FACTOR()*/;
  ...
  //  OGASSERT(n <= _textureAtlas->getCapacity(), "updateAtlasValues: Invalid String length");
    ...
    for(ssize_t i = 0; i < n; i++) {
...
#if OG_FIX_ARTIFACTS_BY_STRECHING_TEXEL
...
#endif // ! OG_FIX_ARTIFACTS_BY_STRECHING_TEXEL
       ...
       // quads[i].tr.vertices.z = 0.0f;
        Color4B c(_displayedColor.r, _displayedColor.g, _displayedColor.b, _displayedColor.a);
        quads[i].tl.colors = c;
        quads[i].tr.colors = c;
        quads[i].bl.colors = c;
        quads[i].br.colors = c;
