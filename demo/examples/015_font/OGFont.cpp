...
class   BMFontConfiguration : public Ref
{
...
BMFontConfiguration::~BMFontConfiguration()
{
	//  OGLOGINFO( "deallocing BMFontConfiguration: %p", this );
	this->purgeFontDefDictionary();
	this->purgeKerningDictionary();
	_atlasName.clear();
	OG_SAFE_DELETE(_characterSet);
}

std::string BMFontConfiguration::description() const
{
	return StringUtils::format("<BMFontConfiguration =   %08lX   | Glphys:%d Kernings:%d | Image = %s>",
		(size_t)this,
...
std::set<unsigned int>* BMFontConfiguration::parseConfigFile(const std::string& controlFile)
{
	...
	if (data[0] == 0)
	{
	//	OGLOG("orange: Error parsing FNTfile %s", controlFile.c_str());
		return nullptr;
	}
...
std::set<unsigned int>* BMFontConfiguration::parseBinaryConfigFile(unsigned char* pData, unsigned long size, const std::string& controlFile)
{
	...
	unsigned long remains = size;
	   ...
	   OGASSERT(pData[3] == 3, "Only version 3 is supported");
	...
	while (remains > 0)
	{
		...
		else if (blockId == 2)
		{
			...
			//OGASSERT(scaleW <= Configuration::getInstance()->getMaxTextureSize() && scaleH <= Configuration::getInstance()->getMaxTextureSize(), "OGLabelBMFont: page can't be larger than supported");

			uint16_t pages = 0; memcpy(&pages, pData + 8, 2);
			OGASSERT(pages == 1, "OGBitfontAtlas: only supports 1 page");
		}
		else if (blockId == 3)
		{
			/*
			...
			const char *value = (const char *)pData;
			       OGASSERT(strlen(value) < blockSize, "Block size should be less then string");
...
void BMFontConfiguration::parseImageFileName(const char* line, const std::string& fntFile)
{
	...
	//  OGASSERT(pageId == 0, "LabelBMFont file could not be found");
	  // file 
	char fileName[255];
...
void BMFontConfiguration::parseInfoArguments(const char* line)
{
	...
	//OGLOG("orange: padding: %d,%d,%d,%d", _padding.left, _padding.top, _padding.right, _padding.bottom);
}
...
void BMFontConfiguration::parseCommonArguments(const char* line)
{
	...
	OGASSERT(value <= maxTextureSize, "OGLabelBMFont: page can't be larger than supported");
	...
	sscanf(tmp, "%d", &value);
	OGASSERT(value == 1, "OGBitfontAtlas: only supports 1 page");
#endif
...
void FontFNT::purgeCachedData()
{
	if (s_configurations)
	{
		...
		OG_SAFE_DELETE(s_configurations);
	}
}
...
FontAtlas * FontFNT::createFontAtlas()
{
	...
	for (auto&& e : _configuration->_fontDefDictionary)
	{
		...
		tempRect = /*OG_RECT_PIXELS_TO_POINTS(*/tempRect/*)*/;
		...
		if (65535 < fontDef.charID) {
			// OGLOGWARN("Warning: 65535 < fontDef.charID (%u), ignored", fontDef.charID);
		}
		else {
