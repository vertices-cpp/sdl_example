...
, _usedGlyphs(GlyphCollection::ASCII)
{
    if (outline > 0.0f)
    {
		_outlineSize = outline;// *OG_CONTENT_SCALE_FACTOR();
...
bool FontFreeType::createFontObject(const std::string &fontName, float fontSize)
{
    ...
    int dpi = 72;
    int fontSizePoints = (int)(64.f * fontSize /** OG_CONTENT_SCALE_FACTOR()*/);
...
FontAtlas * FontFreeType::createFontAtlas()
{
    if (_fontAtlas == nullptr)
    {
        ...
        {
			std::u32string utf32 = iconv_wrapper_to_utf32le(getGlyphCollection(), Utf8ToUtf32le);
            if (!utf32.empty())
            {
...
unsigned char* FontFreeType::getGlyphBitmap(uint64_t theChar, long &outWidth, long &outHeight, Rect &outRect,int &xAdvance)
{
    ...
    do
    {
        ...
        if (_outlineSize > 0 && outWidth > 0 && outHeight > 0)
        {
			...
			auto blendImageMinX = MathMin(outlineMinX, glyphMinX);
			auto blendImageMaxY = MathMax(outlineMaxY, glyphMaxY);
			auto blendWidth =  MathMax(outlineMaxX, glyphMaxX) - blendImageMinX;
			auto blendHeight = blendImageMaxY - MathMin(outlineMinY, glyphMinY);
...
const char* FontFreeType::getGlyphCollection() const
{
    ...
    switch (_usedGlyphs)
    {
    case orange::GlyphCollection::DYNAMIC:
        break;
    case orange::GlyphCollection::NEHE:
        glyphCollection = _glyphNEHE;
        break;
    case orange::GlyphCollection::ASCII:
        glyphCollection = _glyphASCII;
        break;
    case orange::GlyphCollection::CUSTOM:
