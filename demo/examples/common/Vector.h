...
public:
    ...
    : _data()
    {
        static_assert(std::is_convertible<T, Ref*>::value, "Invalid Type for orange::Vector<T>!");
    }
    ...
    {
        static_assert(std::is_convertible<T, Ref*>::value, "Invalid Type for orange::Vector<T>!");
        OGLOGINFO("In the default constructor with capacity of Vector.");
        reserve(capacity);
    }
    ...
    ~Vector<T>()
    {
        OGLOGINFO("In the destructor of Vector.");
        clear();
    }
    ...
    {
        static_assert(std::is_convertible<T, Ref*>::value, "Invalid Type for orange::Vector<T>!");
        OGLOGINFO("In the copy constructor!");
        _data = other._data;
        addRefForAllObjects();
    }
    ...
    {
        static_assert(std::is_convertible<T, Ref*>::value, "Invalid Type for orange::Vector<T>!");
        OGLOGINFO("In the move constructor of Vector!");
    ...
    {
        if (this != &other) {
            OGLOGINFO("In the copy assignment operator!");
            clear();
            _data = other._data;
            addRefForAllObjects();
        }
    ...
    {
        if (this != &other) {
            OGLOGINFO("In the move assignment operator!");
            clear();
            _data = std::move(other._data);
        }
        return *this;
...
//    }
    ...
    {
       OGASSERT( index >= 0 && index < size(), "index out of range in getObjectAtIndex()");
    ...
    {
       OGASSERT(object != nullptr, "The object should not be nullptr");
    ...
    {
         OGASSERT(index >= 0 && index <= size(), "Invalid index!");
         OGASSERT(object != nullptr, "The object should not be nullptr");
    ...
    void popBack()
    {
       OGASSERT(!_data.empty(), "no objects added");
    ...
    {
        OGASSERT(position >= _data.begin() && position < _data.end(), "Invalid position!");
    ...
    {
        OGASSERT(!_data.empty() && index >=0 && index < size(), "Invalid index!");
    ...
    void swap(T object1, T object2)
    {
      ...
      OGASSERT(idx1>=0 && idx2>=0, "invalid object index");
    ...
    {
        OGASSERT(index1 >=0 && index1 < size() && index2 >= 0 && index2 < size(), "Invalid indices");
