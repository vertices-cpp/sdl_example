...
Size Size::operator/(float a) const
{
	OGASSERT(a!=0, "OGSize division by 0.");
 
    return Size(this->width / a, this->height / a);
}
Size Size::operator/(const Size &rhs) const
{
	OGASSERT(rhs.width != 0&&rhs.height!=0, "OGSize division by 0.");
...
{
	setRect(other.x,other.y,this->size.width, this->size.height);
	return *this;
}

Rect & Rect::operator=(const Size & other)
{
	setRect(  this->origin.x, this->origin.y,other.width,other.height);
	return *this;
}
...
void Rect::setRect(float x, float y, float width, float height)
{
	...
	OGASSERT(width >= 0.0f && height >= 0.0f, "width and height of Rect must not less than 0.");
...
bool Rect::intersectsRectStrict(const Rect& rect) const
{
	return !(getMaxX() <= rect.getMinX() ||
		rect.getMaxX() <= getMinX() ||
		getMaxY() <= rect.getMinY() ||
