...
// ============================================================
// 用一块已有的像素内存创建纹理
//   data         : 源像素数据（A8 单通道 或 RGBA/RGB 等）
//   dataLen      : 源数据字节数
//   pixelFormat  : 源像素格式（A8 / RGBA8888 / RGB565 ...）
//   pixelsWide/High : 纹理像素宽高
//   contentSize  : 逻辑尺寸（一般等于宽高；将来支持高清缩放会不同）
// 返回 true 表示创建成功，_texture 已就绪
// ============================================================
	...
	const Size& contentSize)
{
	...
	if (pixelFormat == backend::PixelFormat::A8)
	{
		...
		for (size_t i = 0; i < count; ++i) {
			Uint8 a = src[i];
			rgba[i] = (Uint32)a << 24 | (Uint32)a << 16 | (Uint32)a << 8 | a;
		}
		...
		SDL_UpdateTexture(_texture, nullptr, rgba.data(), pixelsWide * 4);
	}
	else
	{
		Uint32 sdlFmt = toSDLPixelFormat(pixelFormat);
		if (sdlFmt == SDL_PIXELFORMAT_UNKNOWN) return false;

		int bpp = SDL_BITSPERPIXEL(sdlFmt);
		int pitch = pixelsWide * (bpp / 8);

		SDL_Surface* surf = SDL_CreateRGBSurfaceWithFormatFrom(
			(void*)data, pixelsWide, pixelsHigh, bpp, pitch, sdlFmt);
	...
	// ★ 采样模式：立刻设，别等 setAntiAliasTexParameters
	_antialiasEnabled = true;
	SDL_SetTextureScaleMode(_texture, SDL_ScaleModeLinear);
